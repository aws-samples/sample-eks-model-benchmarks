...
variable "manage_cluster" {
  description = <<-EOT
    ...
    When false (brownfield), EKSBench is installed into an EXISTING
...
variable "private_subnet_ids" {
  description = "Private subnets where EKSBench data-plane resources land (RDS, optional ALB, and the subnets Karpenter provisions nodes into). Required when manage_cluster=false."
  type        = list(string)
  default     = []
}
...
variable "install_karpenter_controller" {
  description = <<-EOT
    ...
    required). EKSBench's NodePools + EC2NodeClasses are applied
