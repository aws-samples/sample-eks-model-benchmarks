...
EKSBench's default Terraform provisions an entire EKS cluster plus
...
addons in place, you can install EKSBench alongside your existing
...
everything EKSBench-specific: Aurora Postgres, S3 buckets for
...
- **EKS Pod Identity agent running in the cluster.** EKSBench's
...
- **Karpenter NodePools** (`general-purpose`, `gpu`, `neuron`) and
  ...
  that happen to match EKSBench pod requirements.
...
- Private subnet IDs where EKSBench's data plane should land (must
...
cd terraform
terraform init
terraform plan -out=plan.out       # review the plan — EKSBench creates RDS + Cognito + IAM + NodePools, does NOT touch your cluster addons
...
The three EKSBench NodePools apply a **dedicated taint**:
...
- Operator pods **never** land on EKSBench's nodes (they don't
  tolerate the taint).
- EKSBench pods **never** land on operator nodes (our pods have a
...
- If your cluster has a GPU NodePool whose requirements happen to
  match EKSBench's GPU pod requirements, our `weight: 100` +
...
Because EKSBench's NodePools carry the `accelbench.io/dedicated`
taint, **any DaemonSet that needs to run on EKSBench's GPU or
